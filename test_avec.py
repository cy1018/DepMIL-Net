...
def main():
    ...
    p.add_argument("--data_root", type=str, default=r"E:\cyy_project\M3DFEL\avec14_processed")
    p.add_argument("--split", type=str, default="test", choices=["dev", "test"])
    p.add_argument("--task", type=str, default="Freeform")
    p.add_argument("--label_csv", type=str, default=None)
    ...
    p.add_argument("--use_key_branch", type=int, default=1, choices=[0, 1],
                   help="whether to enable key branch in dual-stream mode")

    p.add_argument("--use_ctx_branch", type=int, default=1, choices=[0, 1],
                   help="whether to enable context branch in dual-stream mode")

    p.add_argument("--fusion_mode", type=str, default="avg",
    ...
    print("[DEBUG] model cfg :",
          f"model={args.model}, "
          f"use_dual_stream={args.use_dual_stream}, "
          f"use_key_branch={args.use_key_branch}, "
          f"use_ctx_branch={args.use_ctx_branch}, "
          f"fusion_mode={args.fusion_mode}, "
          f"score_tau={args.score_tau}, "
          f"use_pgfe={args.use_pgfe}, "
          f"pgfe_type={args.pgfe_type}, "
          f"pgfe_gamma={args.pgfe_gamma}, "
          f"proto_detach={args.proto_detach}")
    ...
    if args.use_dual_stream and (not args.use_key_branch) and (not args.use_ctx_branch):
        raise ValueError("In dual-stream mode, at least one of key/context branch must be enabled.")
    ...
    model_args.use_dual_stream = bool(args.use_dual_stream)
    model_args.use_key_branch = bool(args.use_key_branch)
    model_args.use_ctx_branch = bool(args.use_ctx_branch)
    model_args.fusion_mode = str(args.fusion_mode)
    model_args.score_tau = float(args.score_tau)
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            out = forward_video(model, images).view(-1, 1)
            pred_raw_np = out.detach().cpu().numpy().reshape(-1)
            tgt_np = target.detach().cpu().numpy().reshape(-1)
    ...
    print(f"label_csv : {args.label_csv}")
    print(f"model     : {args.model}")
    print(f"dual      : {args.use_dual_stream}  "
          f"(key={args.use_key_branch}, ctx={args.use_ctx_branch}, "
          f"fusion={args.fusion_mode}, score_tau={args.score_tau})")
    print(f"pgfe      : {args.use_pgfe}  "
          f"(type={args.pgfe_type}, gamma={args.pgfe_gamma}, proto_detach={args.proto_detach})")
    print(f"denorm    : {do_denorm}  ({reason}, source={denorm_source})")
    print(f"MAE={mae:.4f}  RMSE={rmse:.4f}  Pearson={pr:.4f}")
